...
server <- function(input, output, session) {
  ...
  italy <- reactive({
    italy <- italy %>% 
      filter(Year == input$year_sel, House == 'Chamber')
  })
  
  ballot_it_filtered <- reactive({
  ...
  output$map <- renderLeaflet({
    ...
    addPolygons(
      ...
      weight = 1.5,
      label = reg_labels,
      fillColor = colorNumeric(palette = 'YlOrRd',domain = italy$Voter.turnout)
    ) %>%
    addLegend(pal = pal,
              values = italy$Voter.turnout,
              opacity = 0.7,
              title = 'Voter Turnout',
