...
In order to reduce the number of .csv files associated with the project, some operation have been performed on the datasets outside of RStudio. 
...
The "candidate" column was also preemptively removed when present (since it was only used in an handful of election years).
...
# For the other years with "ABRUZZI E MOLISE", separate and use for both regions
...
At this point, as a somewhat secondary step, we would also like to obtain (for both national and regional lists) two data tables in which we will keep only a set percentage of top voted parties for each year. 
...
The interested columns are highlighted in the following table:
...
Using the unique() function we can also notice that the columns "Tipo.data", "Flag.Codes" and "Flags" do not provide any useful information.
...
#All quarterly values for the year column contain letters. We filter them out by trying to convert to integer and then checking if the result is Na
empl_rate <- empl_rate %>%
  filter(
...
Finally, we will calculate the centroid latitude and longitude for each region. This will be later useful to conditionally plot bubbles that are correctly centered to the corresponding regions.
...
In order to have all the data we need in a single table, we will join the geom data that we just gathered with the electoral data that we cleaned at the beginning of this project.
...
For starters, we are going to visually represent the Chamber results coming from the last election year.
The region fill color (Green to Yellow to Red) will correspond to the voter turnout value.
We will later explore how to make our map even more interactive with the help of Shiny.
...
We would now like to try to use our collocation data to fill our regions color.
...
We can now try to utilize the centroid information that we've calculate before to plot bubbles instead of region shapes.
...
In order to be able to dynamically change our map based on Year, House, Shape and Fill Color,  we can proceed to merge everything we have written thus far into a single shiny app.
In this final visualization we will also add a persistent text box that will display information at the national level.
...
shinyApp(ui, server)
```
Now that we have provided a fairly complete and interactive look at our data, we will proceed to display more focused information that was maybe not immediately noticeable in our previous plots.
...
After these initial views on some basic information related to the election years for both Houses, we can start to take a closer look at the parties that were part of said elections.
We will start by plotting the total nr. of parties that were present for each election year (at the national level).
We can also display a linear regression line to better understand how this specific trend is changing.
...
This bar chart allows us to see how the total number of parties per election years has generally been increasing throughout the years. We can also see how election years like 1994 (for Chamber) and 2006/2013 (for both Houses) have a noticeably higher nr of total parties participating in the elections.

A this point, a good idea would be to display the changes in the political spectrum for all regions across all the election years.
A good way to display this information is by using an heatmap.
...
This additional data will allow us to further explore the changes in political views of the Italian population that was introduced in the previous plot. 
...
Firstly, we will create an animated line plot to visually show how these percentages have changed throughout the years.
...
In this visualization, Syncretic rows are kept only for years 2013 and 2018 to account for the rise of M5S.
...
animate(coll_years_anim, fps = 25, duration = 20, width = 800, height = 600, end_pause = 100, renderer=gifski_renderer("./images/collocation_over_years.gif"))
```

This animation reinforces what we discovered in the previous heatmap by clearly showing the two major shifts (here indicated by vertical dotted lines) that happened in the last 25 years.
...
In order to achieve this, we will plot an interactive bar chart that will show in great detail all political collocations. We will use the plotly library to achieve this goal.
To avoid unwanted behaviors in the interactive plot, let's first make sure that every year (and House) has rows for all political collocations. 
...
plot_coll_bar <- function(curr_house) {
  ...
  filter(!grepl(curr_house, House)) %>%
    ...
    layout(
      title = "Political collocation of top voted parties",
...
We will use a simple line plot to visualize the changes related to the employment rates in Italy.
...
In the background, we can also see the parties political collocation for the last years before the two political shift (1992 and 2008) and for the last election year (2018).
...
After collecting and cleaning our datasets we have provided an in-depth interactive map of Italy that contained a range of different information such us: Voter turnout, top voted parties, political collocation and total nr of voters.
We then created a set of more static views that allowed us to give a focused insight on the previously mentioned topics.
...
As a brief final summary, we can list the findings derived from this exploration as follows:
 ...
 - The last 25 years have seen two major shifts in the political views of the country (first from centre/centre-left to centre-right, then from centre-right to syncretic/far-right).
