...
The main goal of this project is to provide a range of visual information regarding the Italian election for both Chamber and Senate.
During the course of this work we will work with multiple election related features on both the national and regional level.
Additionally, we would like to discover if the employment rates in Italy are a relevant factor in influencing the political direction of the country.
...
# TODO - Improve this aberration and add data to Molise
...
server <- function(input, output) {
  ...
  pal <- reactive({
    if(input$fill_sel == 'Voter turnout') {
      colorNumeric(palette = 'RdYlGn',domain = c(min(italy$Voter.turnout), max(italy$Voter.turnout)))
    }
    else if(input$fill_sel == 'Top party collocation') {
      colorFactor('Set2', domain=c('Far-Left','Left','Centre-Left','Centre','Syncretic','Centre-Right','Right','Far-Right'))
    }
  })
...
ggplot(list_it_grouped, aes(x=Year, y=Party, fill=House)) +
  ...
  theme(axis.text.x = element_text(angle = 60, hjust = 1))
```
This bar chart allows us to see how the total number of parties per election years is generally increasing compared to te first elections. We can also see how election years like 1994 (for Cahmber) and 2006/2008 have a noticeably higher nr of total parties participating to the election.
...
ggplot(aggregate_list_reg, aes(x=Year,y=Region,fill=Collocation)) +
  ...
  theme(axis.text.x = element_text(angle = 60, hjust = 1)) 
```
This heatmap allows us to see the radical shift that took place in Italy in the middle of the 90s. We will later try to justify this shift using the employment data that we have previously imported.
...
In order to do it we will plot an interactive bar chart that will show all political collocations. We will use the plotly library to achieve this
...
#  ) +
```
As we might have expected, this changes in political trends corresponds with local minimum values for the employment rates (years 1995 and 2013).
The vertical dotted line represent (just like in the previous line chart) the election years in which a political shift happened in Italy.
