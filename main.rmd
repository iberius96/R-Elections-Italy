...
In order to do this, we are going to plot a bar chart, for both Houses, with one c
...
Another interesting thing we can do with our data is calculating the percentage of voters belonging to/sharing a given political collocation (this will be an estimation over the total population since we only have Collocation information for top voted parties).
...
```{r}
ggplot(list_coll_percent, aes(x=Year, y=Percent, color=Collocation)) + 
  ggtitle("Collocation trends for top parties") +
  geom_point() + 
  scale_x_continuous(breaks=election_years) +
  scale_color_brewer(palette = "Set2") +
  ylab('% of votes') + 
...
#TODO - Next up
#Group left and right -> line plot
#Vertical mirror barchart left/right - far-left/far-right, centre-left/centre-right
```
